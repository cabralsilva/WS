...
extends ControlServices {
    ...
    public RetornoWS<Cliente> updateCliente(Cliente c) throws SQLException {
        RetornoWS<Cliente> retorno = new RetornoWS<Cliente>();
        String sql = "UPDATE Cliente SET \tNome = " + (c.getNome() == null ? null : new StringBuilder("'").append(c.getNome()).append("'").toString()) + ", DataNascimento = " + (c.getDataNascimento() != null ? new StringBuilder("DATE '").append(c.getDataNascimento()).append("'").toString() : null) + ", Cpf = " + c.getCpf() + ", Rg = " + (c.getRg() == null ? null : new StringBuilder("'").append(c.getRg()).append("'").toString()) + ", RazaoSocial = " + (c.getRazaoSocial() == null ? null : new StringBuilder("'").append(c.getRazaoSocial()).append("'").toString()) + ", Cnpj = " + c.getCnpj() + ", InscricaoEstadual = " + (c.getInscricaoEstadual() == null ? null : new StringBuilder("'").append(c.getInscricaoEstadual()).append("'").toString()) + ", Email = '" + c.getEmail() + "'" + ", Cep = " + c.getCep() + ", Logradouro = '" + c.getLogradouro() + "'" + ", Numero = '" + c.getNumero() + "'" + ", Complemento = '" + c.getComplemento() + "'" + ", Bairro = '" + c.getBairro() + "'" + ", Municipio = '" + c.getMunicipio() + "'" + ", Uf = '" + c.getUf() + "'" + ", InformacoesReferencia = '" + c.getInformacoesReferencia() + "'" + ", Ddd[1] = '" + c.getDdd1() + "'" + ", Telefone[1] = '" + c.getTelefone1() + "'" + ", Ddd[2] = '" + c.getDdd2() + "'" + ", Telefone[2] = '" + c.getTelefone2() + "'" + " WHERE " + "Cliente.Codigo = " + c.getCodigoCliente();
    ...
    public RetornoWS<Cliente> updateSenhaCliente(Cliente c) throws SQLException {
        RetornoWS<Cliente> retorno = new RetornoWS<Cliente>();
        String sql = "UPDATE Cliente SET \tSenha = '" + c.getSenhaCliente() + "'" + " WHERE " + "Cliente.Codigo = " + c.getCodigoCliente();
        System.out.println(sql);
        this.sttm.execute(sql);
        retorno.setCodStatus(Long.valueOf(1));
    ...
    public RetornoWS<Cliente> getClienteForCodigo(Long codigo) throws SQLException {
        RetornoWS<Cliente> retorno = new RetornoWS<Cliente>();
        String sql = "SELECT Cliente.Codigo, Cliente.Nome, Cliente.Logradouro, Cliente.Bairro, Cliente.Numero, Cliente.Municipio, Cliente.Cep, Cliente.Uf FROM Cliente WHERE Cliente.Codigo = " + codigo;
        System.out.println(sql);
    ...
    public RetornoWS<Cliente> searchCodigo(Long codigo) throws SQLException {
        RetornoWS<Cliente> retorno = new RetornoWS<Cliente>();
        Cliente c = new Cliente();
        String sql = "SELECT Cliente.Codigo, Cliente.Nome, Cliente.Email, Cliente.Cpf, Cliente.Rg, Cliente.Cnpj, Cliente.RazaoSocial, Cliente.InscricaoEstadual, Cliente.Logradouro, Cliente.Bairro, Cliente.Numero, Cliente.Complemento, Cliente.Municipio, Cliente.Cep, Cliente.Uf, Cliente.Pais, Cliente.Ddd, Cliente.Telefone, Cliente.DataNascimento, Cliente.Sexo, Cliente.DataUltimaCompra, Cliente.Pessoa, Cliente.Ddd, Cliente.Telefone, Cliente.Senha, Cliente.InformacoesReferencia, Cliente.Ddd[2] AS Ddd2, Cliente.Telefone[2] AS Telefone2 FROM Cliente WHERE Cliente.Codigo = " + codigo;
        System.out.println(sql);
        ResultSet rs = this.sttm.executeQuery(sql);
        while (rs.next()) {
            c.setCodigoCliente(Long.valueOf(rs.getLong("Codigo")));
